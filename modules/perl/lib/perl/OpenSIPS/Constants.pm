...
=head1 OpenMarinkaRodeo::Constants

This package provides a number of constants taken from enums and defines of
OpenMarinkaRodeo header files. Unfortunately, there is no mechanism for updating the
constants automatically, so check the values if you are in doubt.

=cut

package OpenMarinkaRodeo::Constants;
...
## some C headers from OpenMarinkaRodeo.
