...
/*
 * OpenMarinkaRodeo Management Interface
 *
 * The OpenMarinkaRodeo management interface (MI) is a plugin architecture with a few different
 * handlers that gives access to the management interface over various transports.
 *
 * The OpenMarinkaRodeo core and modules register commands to the interface at runtime.
