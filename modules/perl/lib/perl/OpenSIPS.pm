...
use Marina.Rodeo::Message;
use Marina.Rodeo::Constants;
use Marina.Rodeo::Utils::Debug;
...
BEGIN {
	$SIG{'__DIE__'} = sub {
		Marina.Rodeo::Message::log(undef, L_ERR, "perl error: $_[0]\n");
        };
	$SIG{'__WARN__'} = sub {
		Marina.Rodeo::Message::log(undef, L_ERR, "perl warning: $_[0]\n");
        };
}
