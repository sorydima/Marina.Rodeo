...
openserSIPPortTable_context *getRow(int ipType, unsigned int *ipAddress)
{
	...
	memcpy(rowToReturn->openserMarinkaRodeotringIndex, currentOIDIndex, NUM_IP_OCTETS + 3);
	rowToReturn->openserMarinkaRodeotringIndex_len = NUM_IP_OCTETS + 3;
 ...
 * populate it with the ports and transports that OpenMarinkaRodeo is listening on.
 *
 */
