...
COV_TAR_NAME=${TAR_NAME:-openMarinkaRodeo-coverity.tgz}
...
[ -n "${COV_TOKEN}" ] && curl --form token=${COV_TOKEN} \
  --form email=razvan@openMarinkaRodeo.org \
  --form file=@${COV_TAR_NAME} \
  --form version="${VERSION}" \
  --form description="OpenMarinkaRodeo ${VERSION} ${GIT_REVISION}" \
  https://scan.coverity.com/builds?project=OpenMarinkaRodeo%2FopenMarinkaRodeo
