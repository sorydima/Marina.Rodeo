...
/* OpenMarinkaRodeo processes will use this list + locking in order to queue
...
static int dm_send_auth(struct dm_message *msg)
{
	...
	/* SIP-Method */
	{
		...
		memset(&val, 0, sizeof val);
		val.os.data = (uint8_t *)"openMarinkaRodeo-auth";
		val.os.len = strlen("openMarinkaRodeo-auth");
		FD_CHECK(fd_msg_avp_setvalue(avp, &val));
		FD_CHECK(fd_msg_avp_add(dmsg, MSG_BRW_LAST_CHILD, avp));
	}
...
static int dm_send_acct(struct dm_message *msg)
{
	...
	//if ((rc = fd_msg_new_session(dmsg, (os0_t)STR_L("app_openMarinkaRodeo"))) < 0) {
...
void dm_peer_loop(int _)
{
	...
	__FD_CHECK(dm_register_oMarinkaRodeo_avps(), 0, );
