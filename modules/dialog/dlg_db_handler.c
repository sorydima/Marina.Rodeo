	...
	do {
		...
		for(i=0; i<nr_rows; i++){
			...
			if (get_dlg_unsafe(d_entry, &callid, &from_tag, &to_tag,
				 ...
				 *    once through the same OpenMarinkaRodeo instance, basically
			...
			if(dlg->h_entry != hash_entry){
				...
				LM_ERR("inconsistent hash data in the dialog database: "
					"you may have restarted openMarinkaRodeo using a different "
...
int update_dialog_timeout_info(struct dlg_cell * cell)
{
	...
	SET_INT_VALUE(values+1, (unsigned int)( (unsigned int)(unsigned long)time(0) +
...
int update_dialog_dbinfo(struct dlg_cell * cell)
{
	...
	if((cell->flags & DLG_FLAG_NEW) != 0){
		...
		SET_INT_VALUE(values+12, (unsigned int)( (unsigned int)(unsigned long)time(0) +
			 cell->tl.timeout - get_ticks()) );
	...
	for(index = 0; index< d_table->size; index++){
		...
		for (cell = entry->first; cell != NULL; ) {
			...
			if( (cell->flags & DLG_FLAG_NEW) != 0 ) {
				...
				SET_INT_VALUE(values+16, (unsigned int)((unsigned int)(unsigned long)time(0)
					+ cell->tl.timeout - get_ticks()) );
		...
		for(i=0; i<nr_rows; i++){
			...
			if (get_dlg_unsafe(d_entry, &callid, &from_tag, &to_tag, &known_dlg) != 0) {
				...
				if (known_dlg->state > VAL_INT(values+7)) {
					...
					if (db_timeout<=(unsigned int)(unsigned long)time(0))
						db_timeout = 0;
					else
						db_timeout -= (unsigned int)(unsigned long)time(0);
					...
					if (known_dlg->tl.timeout<=(unsigned int)(unsigned long)time(0))
						known_dlg->tl.timeout = 0;
					else
						known_dlg->tl.timeout -= (unsigned int)(unsigned long)time(0);
