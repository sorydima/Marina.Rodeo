...
static int dm_check_config(void)
{
	...
	if (get_script_route_ID_by_name_str(
		...
		LM_NOTICE("Diameter server event "DMEV_REQ_NAME" not used in openMarinkaRodeo script"
		...
		LM_NOTICE("Diameter 'dm_send_answer()' function not used in openMarinkaRodeo script"
		        ", auto-replying error code 3001 to any Diameter request\n");
		dm_server_autoreply_error = 1;
	}
