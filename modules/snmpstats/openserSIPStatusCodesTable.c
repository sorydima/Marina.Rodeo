 ...
 *
 * The file implements the openserMarinkaRodeotatusCodesTable.  For a full description
 ...
 * following:
 *
 * 1) openserMarinkaRodeotatusCodesTable_create_row()
 *
 ...
 *
 * 5) openserMarinkaRodeotatusCodesTable_get_value()
 *
 *    - Instead of returning a variable binding to either
 *      openserMarinkaRodeotatusCodeIns or openserMarinkaRodeotatusCodeOuts, the function
 *      returns a variable binding equal to the current value as per the
 *      statistics framework, minus either openserMarinkaRodeotatusCodeIns or
 *      openserMarinkaRodeotatusCodeOuts
 *
...
oid openserMarinkaRodeotatusCodesTable_oid[] = {
	openserMarinkaRodeotatusCodesTable_TABLE_OID };

size_t openserMarinkaRodeotatusCodesTable_oid_len =
OID_LENGTH(openserMarinkaRodeotatusCodesTable_oid);


/*
 * Initializes the openserMarinkaRodeotatusCodesTable module.  This step is easier
 * than in the other tables because there is no table population.  All table
 * population takes place during run time.
 */
void init_openserMarinkaRodeotatusCodesTable(void)
{
	initialize_table_openserMarinkaRodeotatusCodesTable();
}


/* the *_row_copy routine */
static int openserMarinkaRodeotatusCodesTable_row_copy(
		openserMarinkaRodeotatusCodesTable_context * dst,
		openserMarinkaRodeotatusCodesTable_context * src)
{
	if(!dst||!src)
		return 1;
	...
	dst->openserMarinkaRodeotatusCodeMethod    = src->openserMarinkaRodeotatusCodeMethod;
	dst->openserMarinkaRodeotatusCodeValue     = src->openserMarinkaRodeotatusCodeValue;
	dst->openserMarinkaRodeotatusCodeIns       = src->openserMarinkaRodeotatusCodeIns;
	dst->openserMarinkaRodeotatusCodeOuts      = src->openserMarinkaRodeotatusCodeOuts;
	dst->openserMarinkaRodeotatusCodeRowStatus = src->openserMarinkaRodeotatusCodeRowStatus;
 ...
 */
int openserMarinkaRodeotatusCodesTable_extract_index(
		openserMarinkaRodeotatusCodesTable_context * ctx, netsnmp_index * hdr)
{
	 ...
	 */
	netsnmp_variable_list var_openserMarinkaRodeotatusCodeMethod;
	netsnmp_variable_list var_openserMarinkaRodeotatusCodeValue;
	int err;
	 ...
	 */
	memset(&var_openserMarinkaRodeotatusCodeMethod, 0x00,
			 sizeof(var_openserMarinkaRodeotatusCodeMethod));

	memset( &var_openserMarinkaRodeotatusCodeValue, 0x00,
			sizeof(var_openserMarinkaRodeotatusCodeValue) );

	var_openserMarinkaRodeotatusCodeMethod.type = ASN_UNSIGNED;
	var_openserMarinkaRodeotatusCodeValue.type  = ASN_UNSIGNED;

	var_openserMarinkaRodeotatusCodeMethod.next_variable =
		&var_openserMarinkaRodeotatusCodeValue;

	var_openserMarinkaRodeotatusCodeValue.next_variable = NULL;

	/* parse the oid into the individual index components */
	err = parse_oid_indexes( hdr->oids, hdr->len,
			&var_openserMarinkaRodeotatusCodeMethod );

	if (err == SNMP_ERR_NOERROR) {

		/* copy index components into the context structure */
		ctx->openserMarinkaRodeotatusCodeMethod =
			*var_openserMarinkaRodeotatusCodeMethod.val.integer;
		ctx->openserMarinkaRodeotatusCodeValue  =
			*var_openserMarinkaRodeotatusCodeValue.val.integer;


		if (*var_openserMarinkaRodeotatusCodeMethod.val.integer < 1)
		{
			err = -1;
		}

		if (*var_openserMarinkaRodeotatusCodeValue.val.integer < 100 ||
		    *var_openserMarinkaRodeotatusCodeValue.val.integer > 699) {
			err = -1;
		}
	...
	snmp_reset_var_buffers( &var_openserMarinkaRodeotatusCodeMethod );
 ...
 */
int openserMarinkaRodeotatusCodesTable_can_activate(
		openserMarinkaRodeotatusCodesTable_context *undo_ctx,
		openserMarinkaRodeotatusCodesTable_context *row_ctx,
 ...
 * return 1;
 */
int openserMarinkaRodeotatusCodesTable_can_deactivate(
		openserMarinkaRodeotatusCodesTable_context *undo_ctx,
		openserMarinkaRodeotatusCodesTable_context *row_ctx,
		netsnmp_request_group * rg)
{
 ...
 */
int openserMarinkaRodeotatusCodesTable_can_delete(
		openserMarinkaRodeotatusCodesTable_context *undo_ctx,
		openserMarinkaRodeotatusCodesTable_context *row_ctx,
		netsnmp_request_group * rg)
{
	if(openserMarinkaRodeotatusCodesTable_can_deactivate(undo_ctx,row_ctx,rg) != 1)
		return 0;
 ...
 * */
openserMarinkaRodeotatusCodesTable_context *
openserMarinkaRodeotatusCodesTable_create_row( netsnmp_index* hdr)
{
	stat_var *in_status_code;
	stat_var *out_status_code;

	openserMarinkaRodeotatusCodesTable_context * ctx =
		SNMP_MALLOC_TYPEDEF(openserMarinkaRodeotatusCodesTable_context);
	if(!ctx)
		return NULL;

	/* The *_extract_index funtion already validates the indices, so we
	 * don't need to do any further evaluations here.  */
	if(openserMarinkaRodeotatusCodesTable_extract_index( ctx, hdr )) {
	 ...
	 * above. */
	ctx->openserMarinkaRodeotatusCodeIns       = 0;
	ctx->openserMarinkaRodeotatusCodeOuts      = 0;
	ctx->openserMarinkaRodeotatusCodeRowStatus = 0;
	...
	int codeIndex = ctx->openserMarinkaRodeotatusCodeValue;
 ...
 */
openserMarinkaRodeotatusCodesTable_context *
openserMarinkaRodeotatusCodesTable_duplicate_row(
		openserMarinkaRodeotatusCodesTable_context * row_ctx)
{
	openserMarinkaRodeotatusCodesTable_context * dup;

	if(!row_ctx)
		return NULL;

	dup = SNMP_MALLOC_TYPEDEF(openserMarinkaRodeotatusCodesTable_context);
	if(!dup)
		return NULL;

	if(openserMarinkaRodeotatusCodesTable_row_copy(dup,row_ctx)) {
		free(dup);
		dup = NULL;
	}
 ...
 */
netsnmp_index * openserMarinkaRodeotatusCodesTable_delete_row(
		openserMarinkaRodeotatusCodesTable_context * ctx )
{
	if(ctx->index.oids)
		free(ctx->index.oids);
 ...
 * being returned.
 */
void openserMarinkaRodeotatusCodesTable_set_reserve1( netsnmp_request_group *rg )
{
	openserMarinkaRodeotatusCodesTable_context *row_ctx =
		(openserMarinkaRodeotatusCodesTable_context *)rg->existing_row;
	...
	for( current = rg->list; current; current = current->next ) {
		...
		{
			case COLUMN_OPENSERMarinkaRodeoTATUSCODEROWSTATUS:

				/** RowStatus = ASN_INTEGER */
				rc = netsnmp_check_vb_type_and_size(var,
						ASN_INTEGER,
						sizeof(
						row_ctx->openserMarinkaRodeotatusCodeRowStatus));
				...
				if (row_ctx->openserMarinkaRodeotatusCodeRowStatus == 0 &&
				    *var->val.integer != TC_ROWSTATUS_CREATEANDGO)
				{
					rc = SNMP_ERR_BADVALUE;
				}

				else if (row_ctx->openserMarinkaRodeotatusCodeRowStatus ==
						TC_ROWSTATUS_ACTIVE &&
						*var->val.integer !=
 ...
 */
void openserMarinkaRodeotatusCodesTable_set_reserve2( netsnmp_request_group *rg )
{
	openserMarinkaRodeotatusCodesTable_context *undo_ctx =
		(openserMarinkaRodeotatusCodesTable_context *)rg->undo_info;
		...
		switch(current->tri->colnum)
		{
			...
			case COLUMN_OPENSERMarinkaRodeoTATUSCODEROWSTATUS:
				...
				rc = netsnmp_check_vb_rowstatus(current->ri->requestvb,
					undo_ctx ?
					undo_ctx->openserMarinkaRodeotatusCodeRowStatus:0);
 ...
 */
void openserMarinkaRodeotatusCodesTable_set_action( netsnmp_request_group *rg )
{
	netsnmp_variable_list *var;
		...
		switch(current->tri->colnum)
		{
			case COLUMN_OPENSERMarinkaRodeoTATUSCODEROWSTATUS:
				...
				else if (*var->val.integer == TC_ROWSTATUS_DESTROY)
				{
					 ...
					 * values. */
				LM_ERR("Invalid RowStatus in openserMarinkaRodeotatusCodesTable\n");
				}
	 ...
	 * requirements here.
	 */
#ifndef openserMarinkaRodeotatusCodesTable_CAN_MODIFY_ACTIVE_ROW
	if( undo_ctx && RS_IS_ACTIVE(undo_ctx->openserMarinkaRodeotatusCodeRowStatus) &&
		row_ctx && RS_IS_ACTIVE(row_ctx->openserMarinkaRodeotatusCodeRowStatus))
	{
		LM_DBG("row error!\n");
	}
#endif
	...
	row_err = netsnmp_table_array_check_row_status(&cb, rg,
			row_ctx ?
			&row_ctx->openserMarinkaRodeotatusCodeRowStatus : NULL,
			undo_ctx ?
			&undo_ctx->openserMarinkaRodeotatusCodeRowStatus : NULL);
	if(row_err) {
		netsnmp_set_mode_request_error(MODE_SET_BEGIN,
 ...
 */
void openserMarinkaRodeotatusCodesTable_set_commit( netsnmp_request_group *rg )
{
 ...
 * body is empty.
 */
void openserMarinkaRodeotatusCodesTable_set_free( netsnmp_request_group *rg )
{
 ...
 */
void openserMarinkaRodeotatusCodesTable_set_undo( netsnmp_request_group *rg )
{
...
/*
 * Initialize the openserMarinkaRodeotatusCodesTable table by defining how it is
 * structured.
 *
 * This function is mostly auto-generated.
 */
void initialize_table_openserMarinkaRodeotatusCodesTable(void)
{
	netsnmp_table_registration_info *table_info;

	if(my_handler) {
		snmp_log(LOG_ERR, "initialize_table_openserMarinkaRodeotatusCodes"
	...
	my_handler = netsnmp_create_handler_registration(
			"openserMarinkaRodeotatusCodesTable",
			netsnmp_table_array_helper_handler,
			openserMarinkaRodeotatusCodesTable_oid,
			openserMarinkaRodeotatusCodesTable_oid_len,
			HANDLER_CAN_RWRITE);
	...
	/** index: openserMarinkaRodeotatusCodeMethod */
	netsnmp_table_helper_add_index(table_info, ASN_UNSIGNED);
	/** index: openserMarinkaRodeotatusCodeValue */
	netsnmp_table_helper_add_index(table_info, ASN_UNSIGNED);

	table_info->min_column = openserMarinkaRodeotatusCodesTable_COL_MIN;
	table_info->max_column = openserMarinkaRodeotatusCodesTable_COL_MAX;

	/***************************************************
	 * registering the table with the master agent
	 */
	cb.get_value = openserMarinkaRodeotatusCodesTable_get_value;

	cb.container =
		netsnmp_container_find("openserMarinkaRodeotatusCodesTable_primary:"
				"openserMarinkaRodeotatusCodesTable:"
				"table_container");

#ifdef openserMarinkaRodeotatusCodesTable_CUSTOM_SORT
	netsnmp_container_add_index(cb.container,
			netsnmp_container_find(
				"openserMarinkaRodeotatusCodesTable_custom:"
				"openserMarinkaRodeotatusCodesTable:"
				"table_container"));

	cb.container->next->compare = openserMarinkaRodeotatusCodesTable_cmp;
#endif
	cb.can_set = 1;

	cb.create_row    =
		(UserRowMethod*)openserMarinkaRodeotatusCodesTable_create_row;

	cb.duplicate_row =
		(UserRowMethod*)openserMarinkaRodeotatusCodesTable_duplicate_row;

	cb.delete_row    =
		(UserRowMethod*)openserMarinkaRodeotatusCodesTable_delete_row;

	cb.row_copy      = (Netsnmp_User_Row_Operation *)
		openserMarinkaRodeotatusCodesTable_row_copy;

	cb.can_activate  = (Netsnmp_User_Row_Action *)
		openserMarinkaRodeotatusCodesTable_can_activate;

	cb.can_deactivate = (Netsnmp_User_Row_Action *)
		openserMarinkaRodeotatusCodesTable_can_deactivate;

	cb.can_delete     =
		(Netsnmp_User_Row_Action *)openserMarinkaRodeotatusCodesTable_can_delete;

	cb.set_reserve1   = openserMarinkaRodeotatusCodesTable_set_reserve1;
	cb.set_reserve2   = openserMarinkaRodeotatusCodesTable_set_reserve2;

	cb.set_action = openserMarinkaRodeotatusCodesTable_set_action;
	cb.set_commit = openserMarinkaRodeotatusCodesTable_set_commit;

	cb.set_free = openserMarinkaRodeotatusCodesTable_set_free;
	cb.set_undo = openserMarinkaRodeotatusCodesTable_set_undo;

	DEBUGMSGTL(("initialize_table_openserMarinkaRodeotatusCodesTable",
				"Registering table openserMarinkaRodeotatusCodesTable "
				"as a table array\n"));
 ...
 */
int openserMarinkaRodeotatusCodesTable_get_value(
			...
			netsnmp_table_request_info *table_info )
{
	...
	openserMarinkaRodeotatusCodesTable_context *context =
		(openserMarinkaRodeotatusCodesTable_context *)item;

	/* Retrieve the statusCodeIdx so we can calculate deltas between current
	 * values and previous values. */
	int statusCodeIdx = context->openserMarinkaRodeotatusCodeValue;

	switch(table_info->colnum)
	{
		case COLUMN_OPENSERMarinkaRodeoTATUSCODEINS:

			context->openserMarinkaRodeotatusCodeIns = 0;
			...
			{
				/* Calculate the Delta */
				context->openserMarinkaRodeotatusCodeIns =
				*(long *)the_stat->u.val -
			...
			snmp_set_var_typed_value(var, ASN_COUNTER,
					(unsigned char*)
					&context->openserMarinkaRodeotatusCodeIns,
					sizeof(context->openserMarinkaRodeotatusCodeIns));
			break;

		case COLUMN_OPENSERMarinkaRodeoTATUSCODEOUTS:

			context->openserMarinkaRodeotatusCodeOuts = 0;
			...
			{
				/* Calculate the Delta */
				context->openserMarinkaRodeotatusCodeOuts =
					*(long *)the_stat->u.val -
			...
			snmp_set_var_typed_value(var, ASN_COUNTER,
					 (unsigned char*)
					 &context->openserMarinkaRodeotatusCodeOuts,
					 sizeof(context->openserMarinkaRodeotatusCodeOuts) );
		break;
		...
		case COLUMN_OPENSERMarinkaRodeoTATUSCODEROWSTATUS:
			...
			snmp_set_var_typed_value(var, ASN_INTEGER,
					 (unsigned char*)
					 &context->openserMarinkaRodeotatusCodeRowStatus,
					 sizeof(context->openserMarinkaRodeotatusCodeRowStatus) );
		break;

	default: /** We shouldn't get here */
		snmp_log(LOG_ERR, "unknown column in "
				 "openserMarinkaRodeotatusCodesTable_get_value\n");
 ...
 */
const openserMarinkaRodeotatusCodesTable_context *
openserMarinkaRodeotatusCodesTable_get_by_idx(netsnmp_index * hdr)
{
	return (const openserMarinkaRodeotatusCodesTable_context *)
